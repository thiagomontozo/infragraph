...
- **Kubernetes credential theft:** list-only RBAC for eleven allowlisted resource types without Secret, ConfigMap, ServiceAccount, exec/logs, token request, mutation, or cluster-admin access; dedicated account, direct API transport, rotation, and revocation.
