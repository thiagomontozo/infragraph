...
Protocol 1.0 is an outbound-HTTPS, JSON protocol between a separately deployed read-only collector and the InfraGraph control plane. Enrollment uses a short-lived, single-use organization-scoped token. The collector generates an Ed25519 key pair locally, submits only its public key, and receives a collector-bound connector ID plus a separate bearer credential. It signs every inventory snapshot. The control plane binds the credential, organization, collector ID, connector ID/type, public key, monotonically increasing sequence, timestamp, snapshot ID, and content hash before transactionally reconciling a snapshot.
